...
var Coordinates = function (px, py) {
    this.x = null;
    this.y = null;
...
function generateGridLines() {
    ...
    for (var x = 0; x <= stage.width; x += 32) {
        var coord = [x + 0.5, 0.5, x + 0.5, stage.height + 0.5];
        if (x == stage.width) {
            coord[0] -= 1;
            coord[2] -= 1;
        }
    ...
    for (var y = 0; y <= stage.height; y += 32) {
        var coord = [0.5, y + 0.5, stage.width + 0.5, y + 0.5];
    ...
    $j.ajax({
        ...
        success: function (response) {
            ...
            game.map.getTileTypeByCoords = function (x, y) {
                ...
                var arrayIndex = this.getTileIndexByCoords(x, y);
            ...
            if (game.iDataArray != null) {
                for (var i = 0; i < game.iDataArray.objects.length; i++) {

                    var iData = game.iDataArray.objects[i];
                    var item = new SpriteNS.Sprite();
            ...
            if (game.eDataArray != null) {
                for (var i = 0; i < game.eDataArray.objects.length; i++) {

                    var eData = game.eDataArray.objects[i];
                    var enemy = new SpriteNS.Sprite();
            ...
            if (game.tDataArray != null) {
                for (var i = 0; i < game.tDataArray.objects.length; i++) {

                    var tData = game.tDataArray.objects[i];
                    var tool = new SpriteNS.Sprite();
            ...
            var p = game.map.layers.findByProperty("name", "Parameters", true);
...
function getMultiDimensionalMap(originalArray,width) {
    var mdMap = [];
    for (var i = 0; i < originalArray.length; i += width) {
        var smallarray = originalArray.slice(i, i + width);
...
function drawTilex(tileNumber, coords, sign) {
    ...
    if (tileNumber == tileCodes.toll || tileNumber == tileCodes.tollGreen) {
        ...
        var tollText = game.moneyCount - player.inventory.money;

        var offset = 16 - Math.floor(ctx.measureText(tollText).width / 2);
...
function drawTileAbsolute(tileNumber, coords, sign) {
    ...
    var t = getTileCoordsFromImage(tileNumber, sign);

    //ctx.drawImage(assets.devgraphics, t.sx, t.sy, t.swidth, t.sheight, coords.x, coords.y, 32, 32);
...
function getTileCoordsFromImage(tileNumber, sign) {
    ...
    var tileIndex = tileNumber - 1;
    var sx = ((tileIndex + (game.corruption * sign)) % game.map.tilesets[0].indexWidth) * 32;
    var sy = (Math.floor((tileIndex + (game.corruption * sign)) / game.map.tilesets[0].indexWidth)) * 32;
    var swidth = 32;
    var sheight = 32;
...
function drawTileOffset(tileNumber, coords, sign) {
    ...
    var drawC = getTileDrawOffsetCoords(coords);
...
function CheckCollision() {
    ...
    for (var i = 0; i < game.items.length; i++) {
        ...
        for (var j = 0; j < game.tools.length; j++) {


            var tool = game.tools[j];
    ...
    if (game.clock == 0) {
        player.isDead = true;
        var message = randomMessages["time"].getRandomElement();
        game.setDeadMessage(message);
    }
...
function Draw() {
    ...
    // Draw grid lines.
    //        ctx.beginPath();
    //        for (var i = 0; i < assets.gridLineCoordinates.length; i++) {
    //            var lx = assets.gridLineCoordinates[i];
    ...
    var bgTileId = (game.map.parameters.tileset == "dungeon")
        ? tileCodes.futureWall
        : tileCodes.wall
    if (game.map.parameters.wrapAround !== "true") {
        for (var i = 0; i < stage.playboxTileCount; i++) {
            var c = stage.getCoordsByTileIndex(i);
    ...
    var minY = 0;
    var maxY = game.map.layers[0].data.length;
    ...
    for (var i = minY; i < maxY; i++) {
        ...
        var data = game.map.layers[0].data;

        var tileNumber = data[i];
        var coords = game.map.getCoordsByTileIndex(i);
        var tileOffsetCoords = getTileDrawOffsetCoords(coords);
        ...
        //drawTileOffset(tileNumber, coords);
...
function run(bypassTouchscreen) {
    ...
    var continueRunning = doPreWork(bypassTouchscreen);
...
var creditsTextClass = function (text, color, font, alpha, speed, delay, y) {
    this.text = text || "";
    this.color = color || "";
    this.font = font || "";
    this.alpha = alpha || 0;
...
var creditsClass = function () {
    ...
    this.fadeInCredits = function () {
        ...
        if (this.sequence >= 1) {
            ...
            for (var i = 0; i < this.creditsArray.length; i++) {
                var cred = this.creditsArray[i];
...
var gameHudClass = function () {
    ...
    this.Draw = function () {
        ...
        //game.map.parameters.tileset = "dungeon";
    ...
    this.DrawNormalHud = function () {
        ...
        // Draw money count:
        var interval = Math.floor(273 / game.moneyCount);
        interval = Math.min(interval, 13);
        for (var i = 0; i < game.moneyCount; i++) {
        ...
        // Draw keys.
        //ctx.fillText("Keys:", 500, 120);


        var keyDrawIndex = 0;

        var totalKeys = player.inventory.yellowKeys + player.inventory.redKeys + player.inventory.cyanKeys + player.inventory.greenKeys;

        var keyInterval = (totalKeys < 9)
            ? 32
        ...
        for (var i = 0; i < player.inventory.yellowKeys; i++) {

            //drawTileAbsolute(tileCodes.yellowKey, new Coordinates(500 + (i * 32), 170));
            drawTileAbsolute(tileCodes.yellowKey, new Coordinates(500 + (keyDrawIndex * keyInterval), 110));
            keyDrawIndex++;
        }

        // Draw red key inventory.
        for (var i = 0; i < player.inventory.redKeys; i++) {
            //drawTileAbsolute(tileCodes.redKey, new Coordinates(500 + (i * 32), 202));
            drawTileAbsolute(tileCodes.redKey, new Coordinates(500 + (keyDrawIndex * keyInterval), 110));
            keyDrawIndex++;
        }

        // Draw cyan key inventory.
        for (var i = 0; i < player.inventory.cyanKeys; i++) {
            //drawTileAbsolute(tileCodes.cyanKey, new Coordinates(500 + (i * 32), 234));
            drawTileAbsolute(tileCodes.cyanKey, new Coordinates(500 + (keyDrawIndex * keyInterval), 110));
            keyDrawIndex++;
        }

        // Draw green key inventory.
        for (var i = 0; i < player.inventory.greenKeys; i++) {
            //drawTileAbsolute(tileCodes.greenKey, new Coordinates(500 + (i * 32), 266));
            drawTileAbsolute(tileCodes.greenKey, new Coordinates(500 + (keyDrawIndex * keyInterval), 110));
            keyDrawIndex++;
