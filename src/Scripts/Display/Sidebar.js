...
define('Sidebar', [], () => {
  const Sidebar = function Sidebar(game, stage, player, globalDraw) {
    this.style = {
      left: stage.hudCoords.x,
      top: stage.hudCoords.y,
      margin: 20,
    ...
    this.draw = function draw() {
      ...
      if (game.mode === Constants.gameModes.normal) {
        this.drawNormalSidebar();
      } else if (game.mode === Constants.gameModes.title) {
        this.drawTitle();
      } else if (game.mode === Constants.gameModes.password) {
        this.drawPassword();
      }
    };
