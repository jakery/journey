...
function tileDistanceBetween(coords1, coords2) {
    ...
    var yDist = Math.abs(coords1.y - coords2.y);
    return xDist + yDist;
...
SpriteNS.Sprite = function () {
    ...
    this.getTarget = function () {
        var targetPosition = new Coordinates(this.position.x, this.position.y);
    ...
    this.canMove = function (coordinates) {
        ...
        var destinationTileType = game.map.getTileTypeByCoords(destination.x, destination.y);
    ...
    this.checkTile = function () {
        ...
        var tileType = game.map.getTileTypeByCoords(this.position.x, this.position.y);
        ...
        if (tileType == 20) {
            ...
            //if (this.subType != "ball") {
                ...
                if (this.type == "player") {
                    var message = randomMessages["water"].getRandomElement();
...
//                            }
    ...
    this.movePredator = function() {
        var xDist = Math.abs(this.position.x - player.position.x);
                    var yDist = Math.abs(this.position.y - player.position.y);
    ...
    this.crushCheck = function() {
        if (this.type == "player" && !this.canMove(this.position)) {
            this.isDead = true;
            var message = randomMessages["crush"].getRandomElement();
            game.setDeadMessage(message);
        }
    }
        ...
        if (this.subType == "teleporter") {
            if (!sprite.isTeleporting) {
                if (this.destination != null) {
                    var destinationTeleporter = game.items.findByProperty("nameID", this.destination);
    ...
    this.Draw = function () {
        ...
        if (this.type == "player") {
            ...
            var tileNumber = this.tileGraphic;
